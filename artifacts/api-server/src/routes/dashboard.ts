...
router.get("/dashboard", async (req, res) => {
  ...
  const healthScore = monthlyIncome > 0
    ? Math.min(100, Math.max(0, Math.round((monthlySavings / monthlyIncome) * 100 + 50)))
    : 50;
  ...
  const recentExpenses = await db.select().from(expensesTable)
    ...
    .limit(10);
  const recentIncome = await db.select().from(incomeTable)
    .where(eq(incomeTable.userId, userId))
    .orderBy(sql`${incomeTable.date} DESC, ${incomeTable.createdAt} DESC`)
    .limit(10);
