...
export default function ExpensesPage() {
  ...
  function openEdit(e: Expense) { setEditItem(e); setForm({ amount: e.amount.toString(), category: e.category, note: e.note ?? "", date: e.date }); setShowModal(true); }
  ...
  const handleExport = async () => {
    ...
    const q = new URLSearchParams({ type: "expenses", ...(startDate ? { startDate } : {}), ...(endDate ? { endDate } : {}) });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => setShowModal(true)}><Plus className="h-4 w-4 mr-1.5" />Add Expense</Button>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div>
              ...
              {data.data.map((e) => (
                <div key={e.id} className="flex flex-col sm:grid sm:grid-cols-5 gap-2 sm:gap-4 px-4 py-3 border-b border-border/50 hover:bg-accent/30 transition-colors last:border-0">
