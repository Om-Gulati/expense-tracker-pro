    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {loadingTrends ? (
          ...
          <>
            <StatTile label="Daily Avg Spend" value={format(trends?.dailyAverage ?? 0)} icon={TrendingDown} iconClass="text-destructive" />
            <StatTile label="Weekly Avg Spend" value={format(trends?.weeklyAverage ?? 0)} icon={Calendar} iconClass="text-amber-500" />
      ...
      <Card>
        ...
        <CardContent>
          {loadingMonthly ? <Skeleton className="h-64 w-full" /> : !monthly?.length ? (
            <p className="text-sm text-muted-foreground py-10 text-center">No data yet. Add some income and expenses to see trends.</p>
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={monthly} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} />
                <YAxis tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} tickFormatter={formatTick} width={50} />
                <Tooltip
      ...
      <Card>
        ...
        <CardContent>
          {loadingMonthly ? <Skeleton className="h-52 w-full" /> : !monthly?.length ? (
                ...
                {categories.slice(0, 8).map((c, i) => (
                  <div key={c.category}>
                    ...
                    <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${c.percentage}%`, background: COLORS[i % COLORS.length] }} />
                    </div>
                  </div>
                ))}
              </div>
