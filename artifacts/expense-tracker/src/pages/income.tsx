  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-8 text-center"><p className="text-sm text-muted-foreground">No income records yet.</p></div>
          ) : (
            <div>
              ...
              {data.data.map((i) => (
                <div key={i.id} className="flex flex-col sm:grid sm:grid-cols-5 gap-2 sm:gap-4 px-4 py-3 border-b border-border/50 hover:bg-accent/30 transition-colors last:border-0">
                  ...
                  <span className="text-sm font-semibold text-primary sm:text-right">{formatCurrency(i.amount)}</span>
      ...
      <Dialog open={showModal} onOpenChange={(o) => { if (!o) closeModal(); }}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5"><Label>Amount</Label><Input type="number" step="0.01" min="0.01" placeholder="0.00" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} required /></div>
            ...
            <div className="space-y-1.5"><Label>Source (optional)</Label><Input placeholder="Employer, client, platform..." value={form.source} onChange={e => setForm(f => ({ ...f, source: e.target.value }))} /></div>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(o) => { if (!o) setDeleteId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete income?</AlertDialogTitle><AlertDialogDescription>This action cannot be undone.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
